-- [[ PRO PANEL V8 - THE ULTIMATE STABLE EDITION ]]
local CoreGui = game:GetService("CoreGui")
if CoreGui:FindFirstChild("GaoProV8") then
    CoreGui:FindFirstChild("GaoProV8"):Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "GaoProV8"
...
local ghostPos, ghostClone = nil, nil
AddToggle(MainMenu, "เดินทะลุ/ghost", function(s)
    ...
    if ghostActive then
        ...
        for _, v in pairs(ghostClone:GetChildren()) do if v:IsA("BasePart") then v.Anchored = true v.CanCollide = false end end
        for _, v in pairs(char:GetChildren()) do if v:IsA("BasePart") then v.Transparency = 0.5 v.CanCollide = false end end
    else
        if ghostPos then char.HumanoidRootPart.CFrame = ghostPos end
        if ghostClone then ghostClone:Destroy() ghostClone = nil end
        for _, v in pairs(char:GetChildren()) do if v:IsA("BasePart") then v.Transparency = 0 v.CanCollide = true end end
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if noclip and LocalPlayer.Character then
        ...
        if not antiFall then
            ...
            antiFall.Size = Vector3.new(10000, 1, 10000); antiFall.Position = Vector3.new(0, -100, 0)
            antiFall.Anchored = true; antiFall.Transparency = 0.8; antiFall.Color = Color3.new(1,0,0)
        end
    elseif antiFall then antiFall:Destroy() antiFall = nil end
    
    -- ESP logic for Players & Bots
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("Humanoid") and v.Parent ~= LocalPlayer.Character then
            ...
            if _G.Chams then
                ...
                highlight.FillColor = isPlayer and Color3.new(0, 1, 0) or Color3.new(1, 0, 0)
            ...
            if (_G.Box or _G.Line) then
                if not espRoot then
                    ...
                    box.Color3 = isPlayer and Color3.new(0,1,0) or Color3.new(1,0,0)
...
AddInfo('Status: <font color="rgb(0,255,0)">ONLINE V8</font>')
